...
//-------------------------------------------------------------------
int shortestPath(Graph graph, int start, int stop, JRB d, JRB parent) {
	JRB visited = make_jrb();
	Dllist node;
	Dllist queue = new_dllist();
	int n, output[100], i, u, v, min_length = 0;

	jrb_insert_int(d, start, new_jval_i(0));
	jrb_insert_int(parent, start, new_jval_i(start));
	...
	while ( !dll_empty(queue) ) {
		...
		min_length = jval_i(jrb_find_int(d, u)->val);
		...
		if (jrb_find_int(visited, u) == NULL) { 
			...
			for (i = 0; i < n; i++) {
				...
				if (jrb_find_int(visited, v) == NULL) {
					...
					JRB temp = jrb_find_int(d, output[i]);

					if (temp == NULL) {
						jrb_insert_int(d, v, new_jval_i(min_length + 1));
