...
int main(int argc, char *argv[]){
    ...
    gtk_widget_set_name(dict.remove_word_btn, "hintTable");
    gtk_widget_set_size_request(dict.table_entry, 390, 400);
    ...
    gtk_grid_attach(GTK_GRID(grid), dict.table_entry, 1, 2, 1, 1);
    gtk_grid_attach(GTK_GRID(grid), dict.add_word_btn, 1, 3, 1, 1);
    gtk_grid_attach(GTK_GRID(grid), dict.search_word_btn, 1, 4, 1, 1);
    gtk_grid_attach(GTK_GRID(grid), dict.remove_word_btn, 1, 5, 1, 1);
    gtk_grid_attach(GTK_GRID(grid), dict.set_default_btn, 1, 6, 1, 1);
    ...
    {
        widgets.entry[x] = gtk_entry_new();
        gtk_widget_set_name(widgets.entry[x], "itemTable");
        gtk_table_attach_defaults (GTK_TABLE(dict.table_entry), widgets.entry[x] , 0, 1, x, x+1);
    }
...
void set_fields_default(GtkEntry * widget){
  ...
  for (int x = 0; x < TABLE; x++)
    {
        gtk_entry_set_text(widgets.entry[x], "");
    }
}
void add_word_handler(void) {
...
void suggestionWord(BTA *book, char new_word[]){
  ...
  while (btseln(book, word, mean, dsize, &rsize) == 0)
  {
    ...
    if (strcmp(n, m) == 0){
      ...
      if (soundex_index < 10){
        printf("%s\n", word);
        gtk_entry_set_text(GTK_ENTRY(widgets.entry[soundex_index]), strdup(word));
      }
