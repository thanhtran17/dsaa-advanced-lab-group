...
char word[256];
char mean[256];
char soundex_word[10][256];
int soundex_index = 0;
char container_word[1000000][256];
...
int main(int argc, char *argv[]){
    ...
    /*        Event Handler         */
    g_signal_connect(dict.word_entry, "key_press_event", G_CALLBACK(input_word_handler), NULL);
...
gboolean input_word_handler( GtkWidget *widget, GdkEventKey *event ) {
    ( void ) widget;
    //strcpy(word, gtk_entry_get_text(dict.word_entry));
    //printf("%s\n", word);
    //suggestionWord(word);
    //printf("Length suggestion: %d\n", soundex_index);
      guint keys[55] =
    {
    ...
    for ( guint i = 0 ; i < 55 ; i++ )
    {
        ...
        else if (event->keyval == keys[54]){
          strcpy(word, soundex_word[0]);
          gtk_entry_set_text(dict.word_entry, word);
          return FALSE;
        }
    }
...
void set_fields_default(GtkEntry * widget){
  (void) widget;
  gtk_entry_set_text(dict.word_entry, "");
  gtk_entry_set_text(dict.mean_entry, "");
}
void add_word_handler(void) {
  strcpy(word, gtk_entry_get_text(dict.word_entry));
  strcpy(mean, gtk_entry_get_text(dict.mean_entry));
  if (strlen(word) > 0 && strlen(mean) > 0){
    ...
    g_printf("Please input word and mean for adding\n");
  }
}
void search_word_handler(void) {
  strcpy(word, gtk_entry_get_text(dict.word_entry));
  if (strlen(word) > 0){
    ...
    g_printf("Please input word for searching\n");
  }
}
void remove_word_handler(void){
  strcpy(word, gtk_entry_get_text(dict.word_entry));
...
void findWord(BTA *head_node,int dsize, int *rsize)
{
  if (btsel(head_node, word, mean, dsize, rsize) == 0)
  {
    ...
    gtk_entry_set_text(dict.mean_entry, mean);
  }
  else
  {
...
void suggestionWord(BTA *book, char new_word[]){
  ...
  strcpy(current_word, soundex(strdup(new_word)));
  if (btpos(book, 1) != 0)
  {
    printf("Some errors happened!\n");
  }
